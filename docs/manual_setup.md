...
- Manual camera setup:
    ...
    2. Run `bash setup_camera.sh` from the `VectorBlox-SDK-release-v3.0/example/soc-video-c` directory. This script only needs to be run once, and only if the camera fails to function properly after running `quick_start_3_0.sh`.
...
- Run the following commands in the `VectorBlox-SDK-release-v3.0/example/soc-video-c` directory:
